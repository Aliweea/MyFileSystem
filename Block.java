...
public class Block {
    ...
    public Block(String name, File file, boolean rec) throws IOException {
        blockName = name;
        blockFile = file;
        blockBitMap = new File(blockFile.getPath() + File.separator + "BitMap&&Fat.txt");
        recover = new File(blockFile.getPath() + File.separator + "recover.txt");
        fcbFile = new File(blockFile.getPath() + File.separator + "FCB.txt");
        ...
        if (!rec) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, "The files aren't compelete. " +
                		"You can choose another place or delete \"myFileSystem\" " +
                		 "in this dir and run this again!", "Error", JOptionPane.ERROR_MESSAGE);
                System.exit(0);
            }
        }
    }
    ...
    public void putFCB(File file, double capacity) throws IOException {
        fcbWriter = new FileWriter(fcbFile, true);		//×·¼ÓÐ´ÈëFCB
        fcbWriter.write("Name: " + file.getName() + "\r\n");
        fcbWriter.write("Path: " + file.getPath() + "\r\n");
        fcbWriter.write("Capacity: " + String.valueOf(capacity) + "\r\n");
        String ctime = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date(file.lastModified()));
        fcbWriter.write("Date last updated: " + ctime + "\r\n");
        fcbWriter.write("-------------------------- ------------------------------\r\n");
        fcbWriter.close();
    }
    ...
    public boolean deleteFile(File file, double capacity){
        if (file.getName().equals("CÅÌ") || file.getName().equals("DÅÌ") || file.getName().equals("EÅÌ") || file.getName().equals("FÅÌ")
              || file.getName().equals("FCB.txt")  || file.getName().equals("BitMap&&Fat.txt")|| file.getName().equals("recover.txt")){
    ...
    // Get space
    public static double getSpace(File file){
        double space = 0;
        try {
